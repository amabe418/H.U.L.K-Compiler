...
%struct.A = type { i8 }
@.str.6919333181322027406 = private unnamed_addr constant [2 x i8] c"A\00"
%struct.B = type { %struct.A* }
@.str.4199750530058918380 = private unnamed_addr constant [2 x i8] c"B\00"
define %struct.A* @construct_A() {
entry:
  %obj_0_raw = call i8* @malloc(i32 8)
  %obj_0 = bitcast i8* %obj_0_raw to %struct.A*
  ret %struct.A* %obj_0
}

define i8* @A_f(%struct.A* %self) {
entry:
  ret i8* @.str.6919333181322027406
}

define %struct.B* @construct_B() {
entry:
  %obj_1_raw = call i8* @malloc(i32 8)
  %obj_1 = bitcast i8* %obj_1_raw to %struct.B*
  %base_obj_2 = call %struct.A* @construct_A()
  %base_ptr_field_3 = getelementptr %struct.B, %struct.B* %obj_1, i32 0, i32 0
  store %struct.A* %base_obj_2, %struct.A** %base_ptr_field_3
  ret %struct.B* %obj_1
}

define i8* @B_f(%struct.B* %self) {
entry:
  ret i8* @.str.4199750530058918380
}

define i32 @main() {
entry:
  %unary_4 = fneg double 5.000000
  %x_5 = alloca double
  store double %unary_4, double* %x_5
  %load_6 = load double, double* %x_5
  %boxed_op_7 = fcmp ugt double %load_6, 0.000000
  br i1 %boxed_op_7, label %then_8, label %else_9

then_8:
  %new_obj_11 = call %struct.A* @construct_A()
  br label %ifcont_10

else_9:
  %new_obj_12 = call %struct.B* @construct_B()
  br label %ifcont_10

ifcont_10:
  %iftmp_13 = phi i1 [ %boxed_op_7, %then_8 ], [ %boxed_op_7, %else_9 ]
  %y_14 = alloca i1
  store i1 %iftmp_13, i1* %y_14
  %load_15 = load %struct.B*, %struct.B** %y_14
  %method_call_16 = call i8* @B_f(%struct.B* %load_15)
