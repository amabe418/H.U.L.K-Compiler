...
void LLVMCodeGenerator::generateVTable(TypeDecl *typeDecl)
{
    ...
    for (const std::string &methodName : allMethods)
    {
        ...
        // CRITICAL FIX: Search for the MOST SPECIFIC method implementation
        // Start from the current type and work up the inheritance chain
        std::vector<std::string> chain = getInheritanceChain(typeDecl->name);

        // Search from most specific (current type) to least specific (base types)
        for (const std::string &searchType : chain)
        {
            functionName = searchType + "_" + methodName;
            methodFunc = module_->getFunction(functionName);
            if (methodFunc)
            {
                std::cout << "[LLVM CodeGen] Found method " << functionName << " for " << typeDecl->name << " (most specific implementation)" << std::endl;
                break;
            }
        }
...
void LLVMCodeGenerator::generateMethodFunction(MethodDecl *methodDecl)
{
    ...
    // CRITICAL FIX: Store the current type context before processing method
    std::string methodTypeContext = current_type_;
    std::cout << "[LLVM CodeGen] Method context type: " << methodTypeContext << std::endl;

    // Create method function signature
    std::vector<llvm::Type *> paramTypes;

    // First parameter is always 'self' (pointer to the instance)
    llvm::StructType *instanceType = getOrCreateStructType(methodTypeContext);
    ...
    std::string methodName = methodTypeContext + "_" + methodDecl->name;
    ...
    // CRITICAL FIX: Set the correct type context for this method
    current_type_ = methodTypeContext;
    ...
    for (auto &arg : method->args())
    {
        if (argIndex == 0)
        {
            ...
            // CRITICAL FIX: Track the object type for 'self' variable
            current_scope_->variable_object_types["self"] = methodTypeContext;
            trackVariableType("self", methodTypeContext);

            std::cout << "[LLVM CodeGen] Set self variable type to: " << methodTypeContext << std::endl;
        }
        else
        {
