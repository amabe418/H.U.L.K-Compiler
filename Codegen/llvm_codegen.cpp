...
void LLVMCodeGenerator::visit(BinaryExpr *expr)
{
    ...
    std::cout << "  - Left: " << (leftIsBoxed ? "BoxedValue*" : leftIsNative ? "Native"
                                                                             : "Other")
              << std::endl;
    std::cout << "  - Right: " << (rightIsBoxed ? "BoxedValue*" : rightIsNative ? "Native"
                                                                                : "Other")
              << std::endl;
...
void LLVMCodeGenerator::visit(MethodCallExpr *expr)
{
    ...
    if (needsDynamicDispatch && !objectTypeName.empty())
    {
        ...
        // CRITICAL FIX: Find the MOST SPECIFIC type that defines this method
        // Start from the object's actual type and work up the inheritance chain
        std::string baseTypeForMethod = objectTypeName;

        // Get the inheritance chain for the object's type
        std::vector<std::string> chain = getInheritanceChain(objectTypeName);

        // Search from most specific (actual type) to least specific (base types)
        for (const std::string &searchType : chain)
        {
            auto methodsIt = type_methods_.find(searchType);
            if (methodsIt != type_methods_.end())
            {
                ...
                if (hasMethod)
                {
                    baseTypeForMethod = searchType;
                    std::cout << "[LLVM CodeGen] Found method " << expr->methodName << " in type " << searchType << " (most specific)" << std::endl;
                    break; // Use the most specific implementation
                }
            }
        }
...
void LLVMCodeGenerator::visit(SelfExpr *expr)
{
    ...
    while (scope)
    {
        ...
        if (it != scope->variables.end())
        {
            ...
            // CRITICAL FIX: Track the object type for the loaded self value
            auto objTypeIt = scope->variable_object_types.find("self");
            if (objTypeIt != scope->variable_object_types.end())
            {
                std::string objectType = objTypeIt->second;
                trackValueType(current_value_, objectType);
                trackVariableType("self", objectType);
                std::cout << "[LLVM CodeGen] SelfExpr: Found self variable in scope with type: " << objectType << std::endl;
            }
            else
            {
                // Fallback: use current_type_ if available
                if (!current_type_.empty())
                {
                    trackValueType(current_value_, current_type_);
                    trackVariableType("self", current_type_);
                    std::cout << "[LLVM CodeGen] SelfExpr: Using current_type_ for self: " << current_type_ << std::endl;
                }
                else
                {
                    std::cout << "[LLVM CodeGen] SelfExpr: Found self variable in scope (no type info)" << std::endl;
                }
            }
            return;
        }
